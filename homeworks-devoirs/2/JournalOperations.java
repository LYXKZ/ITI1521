...
/**
 * la classe JournalOperations qui est la classe principale produisant les
 * rapports et sorties voulus
 * 
 * @author Livaniaina Rakotomalala (lrakotom@uottawa.ca)
 * @version 02/10/2024
 * 
 *          NOTE: ***Vous n avez pas le droit de changer le contenu de cette
 *          classe***
 */
public class JournalOperations {
    ...
    public static void resolveAndAddBankAccount(Bank bank, Person owner, String accountAndBalanceAndRate,
            String separator) {
     ...
     *             args[0] prendra le nom du fichier input contenant la liste des
     *             transactions de la banque
     *             args[1] prendra le nom du fichier output contenant le rapport des
     *             transactions journalieres de la banque
     * @throws Exception
     */
        ...
        try (BufferedReader br = new BufferedReader(fileReader)) {
            ...
            while ((ligne = br.readLine()) != null && ligne.length() != 0) {
                ...
                if (operationType.equalsIgnoreCase("bank")) {
                    // LocalDateTime datetime = Utils.convertStringToDateTime(values[0]); //on n en
                    // a aps besoin pour le moment
                    int index = 2;
                    ...
                    switch (action) {
                        ...
                        case "new-client":
                            ...
                            for (int i = 0; i < numberSavingAccount.intValue(); i++) {
                                ...
                                BankAccount bankAccount = new SavingsAccount(tdInstitution, transitNumber, client,
                                        interestRate);
                                bank.configureBankAccount(bankAccount, transitNumber, true);
                            }
                            break;
